...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        Arguments:
            ...
            scales * scales = hs * ws
            aspect_ratios = hs / ws
            hs * hs = aspect_ratios * scales * scales -> h_ratios = sqrt(aspect_ratios)
            ws * ws = scales * scales * aspect_ratios -> w_ratios = 1 / sqrt(aspect_ratios)
        """
        ...
        # 生成的anchors模板都是以（0, 0）为中心的, shape [len(ratios)*len(scales), 4] 计算左上和右下坐标
